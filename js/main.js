 ...
 */
let replaceCSSComments = (str) => str.replace(/\/\*.*?\*\/\s?/g, "").trim();
 ...
 */
let replaceHTMLComments = (str) => str.replace(/<!-.*?->\s*/g, "").trim();
 ...
 */
let validateFileType = (str) => /.(\.jpg|\.jpeg|\.png)$/.test(str);
 ...
 */
let checkYear = (year) => /^(19\d\d|20\d\d|2100)$/.test(year);
...
let checkEmail = (email) =>
  /^[a-zA-Z]+\W?[a-zA-Z]+@[a-zA-Z\.]+\.[a-z]{2,3}$/.test(email);
 ...
 */
let checkDomainUrl = (domain) =>
  /^https?:\/\/+([a-zA-Z0-9-_]+\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\.[a-zA-Z]{2,5}?$/.test(
    domain
  );
console.log(
